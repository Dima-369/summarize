export type LlmProvider = "xai" | "openai" | "google" | "anthropic" | "zai" | "nvidia" | "qwen";
...
const PROVIDERS: LlmProvider[] = ["xai", "openai", "google", "anthropic", "zai", "nvidia", "qwen"];
...
export function normalizeGatewayStyleModelId(raw: string): string {
  ...
  if (!PROVIDERS.includes(provider as LlmProvider)) {
    throw new Error(
      `Unsupported model provider "${provider}". Use xai/..., openai/..., google/..., anthropic/..., zai/..., nvidia/..., or qwen/...`,
    );
  }
